...
Для сборки статической библиотеки с функцией `ciadpi_run` выполните:

```
make lib
```

Полученный файл `libciadpi.a` можно использовать в сторонних проектах. При
компиляции под macOS или iOS следует использовать кросс-компилятор clang,
например:

```
make lib CC="clang -target arm64-apple-ios"
```

При линковке достаточно подключить `ciadpi.h` и вызвать `ciadpi_run(argc, argv)`.
